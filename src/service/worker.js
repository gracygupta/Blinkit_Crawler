...
const { storeProductsInFirestore } = require('../config/firebase.config');
const logger = require('../config/logger');
const { delay } = require('./general.helper');


// Function to fetch product details for a category with exponential backoff
async function fetchProductDetails(city, l0_cat, l1_cat, retryCount = 0) {
    const maxRetries = 5;
    ...
    try {
        ...
        await storeProductsInFirestore(city, productDetails);
...
(async () => {
    const { city, l0_cat, l1_cat } = workerData;

    try {
        const productDetails = await fetchProductDetails(city, l0_cat, l1_cat);
